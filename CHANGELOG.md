...
1. При аварии Core 1 (loop) пишет лог на SD карту через `logger.log()`
2. SD карта использует SPI шину (50-200мс на операцию)
3. В это время пользователь нажимает F5
4. Core 0 (Network Task) пытается открыть `/www/index.html` с SD
5. **SPI шина занята → Core 0 блокируется!**

### Решение
**index.html перемещён в PROGMEM (flash память):**
- Оригинал: 105 КБ
- Gzip сжатие: 19.2 КБ (18% от оригинала)
- Обслуживание из PROGMEM устраняет доступ к SD для главной страницы

**Преимущества:**
1. Главная страница загружается мгновенно (нет SD задержки)
2. Нет конфликта SPI шины во время записи логов
3. Web работает даже при активной записи на SD

### Изменённые файлы
- **index_html_gz.h**: новый файл с gzipped HTML в PROGMEM
- **AppNetwork.cpp**: обслуживание "/" из PROGMEM вместо SD

### Остаточные риски
JS/CSS файлы (`/www/js/*`) всё ещё на SD карте, но они загружаются только при первом открытии страницы, а не при каждом F5.
