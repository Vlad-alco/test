...
### Задача
Унифицировать отображение данных в блоке РАСЧЕТЫ:
- Убрать дублирование скорости отбора
- Привести к единому порядку: расчётное / текущее (цель / факт)
- Добавить факт объёма голов в строку ГОЛОВЫ

### Решение

#### Изменения в блоке ГОЛОВЫ:

**Было:**
```
РАСЧЕТЫ
ГОЛОВЫ: 500 мл                          2667 мл/ч
Метод: Kss                              Подэтап: Spit
Скорость: 200/2667 мл/ч                (расч/тек)
Объём: 45/150 мл                       Время подэтапа: 05:00
```

**Стало:**
```
РАСЧЕТЫ (расч/тек)
ГОЛОВЫ: 500/85 мл                       200/2667 мл/ч
Метод: Kss                              Подэтап: Spit
Объём подэтапа: 150/45 мл              Время подэтапа: 05:00
```

#### Порядок значений (везде одинаковый):

| Поле | Порядок | Источник |
|------|---------|----------|
| ГОЛОВЫ объём | цель/факт | `headsTotalTarget` / `headsVolDone` |
| Скорость | расч/тек | `headsSpeedCalc` / `headsSpeed` |
| Объём подэтапа | цель/факт | `headsVolTarget` / `headsVolSub` |

#### Изменённые файлы
- **index.html**:
  - Заголовок: "РАСЧЕТЫ" → "РАСЧЕТЫ (расч/тек)"
  - ГОЛОВЫ: добавлен факт объёма (`headsVolDone`)
  - Скорость перенесена в строку ГОЛОВЫ в формате расч/тек
  - Удалена дублирующая строка "Скорость: ..."
  - "Объём:" → "Объём подэтапа:"
  - Обновлён JavaScript для новых элементов

---

## [2025-03-16] — Сессия 17

### Задача
Дополнить Web интерфейс для этапа ГОЛОВЫ:
- Отображать две скорости: расчётная / текущая (мл/ч)
- Отображать объём подэтапа: расчётный / фактически отобрано (мл)
- Переименовать "Осталось:" в "Время подэтапа:"

### Решение

#### Добавлены новые поля в SystemStatus:
```cpp
...
```
ГОЛОВЫ: 425 мл
Скорость: 200/500 мл/ч (расч/тек)
Объём: 30/206 мл (факт/расч)
Время подэтапа: 05:00
```

#### Изменённые файлы
- **ProcessCommon.h**: добавлены `headsVolSub`, `headsVolTarget`
- **ProcessEngine.h**: добавлена переменная `headsVolSub`
- **ProcessEngine.cpp**: 
  - Расчёт `headsVolSub` в `handleGolovy()`
  - Сброс `headsVolSub` и установка `headsVolTarget` в функциях старта подэтапов
- **AppNetwork.cpp**: добавлены `headsVolSub`, `headsVolTarget` в JSON
- **index.html**:
  - Добавлены элементы `mon-head-speed-calc`, `mon-head-speed-real`
  - Обновлён JavaScript для отображения новых полей
  - Переименовано "Осталось" → "Время подэтапа"

---

## [2025-03-16] — Сессия 16

### Проблема
Web интерфейс **всё ещё зависает** при аварии TSA, даже после исправления setTimeout.

### Симптомы
- LCD показывает аварию корректно
- Web полностью зависает (F5 не загружает страницу)
- Монитор порта показывает нормальную работу Core 1

### Анализ
**Корневая причина: Конфликт SPI шины между ядрами!**

```
Авария TSA → Core 1 (loop) пишет лог на SD (SPI занят 50-200мс)
    ↓
Пользователь нажимает F5 → Core 0 (Network Task) пытается открыть index.html с SD
    ↓
SPI шина занята → Core 0 блокируется! Web висит!
```

### Решение
**FreeRTOS мьютекс для SD карты:**
- Создан глобальный мьютекс `sdMutex`
- Все операции с SD обёрнуты в `xSemaphoreTake` / `xSemaphoreGive`
- Если одно ядро работает с SD — другое ждёт освобождения

### Как работает мьютекс
```
Core 1: logger.log() → ЗАХВАТИТЬ мьютекс → SD.open() → пишу... → ОСВОБОДИТЬ
Core 0: GET /        → ЗАХВАТИТЬ мьютекс → занят! → ЖДУ... → OK! → читаю → ОСВОБОДИТЬ
```

### Изменённые файлы
- **BuhloWar110326CL2core.ino**: создание `sdMutex = xSemaphoreCreateMutex()`
- **SDLogger.h**: 
  - Добавлен `extern SemaphoreHandle_t sdMutex`
  - Класс `SDScopeLock` для автоматического захвата/освобождения
  - Методы `log()` и `readLastLog()` защищены мьютексом
- **AppNetwork.cpp**: 
  - Обработчик "/" защищён мьютексом (оба режима: STA и AP)
...
### Исправлено
1. **client.setTimeout(2000) → setTimeout(2)**
   - WiFiClient::setTimeout() принимает СЕКУНДЫ, не миллисекунды!
   - Было: 2000 секунд = 33 минуты блокировки!
   - Стало: 2 секунды

2. **Интервал аварий TSA: 60 сек → 30 сек**
   - Унифицировано с BOX (30 сек)

### Причина зависания Web при аварии
- `bot->sendMessage()` — блокирующий вызов (~1-2 сек)
- При setTimeout(2000) сек = 33 мин блокировка!
- Теперь setTimeout(2) сек — максимум 2 сек ожидание

### Изменённые файлы
- **AppNetwork.cpp**: setTimeout(2), интервал аварий 30 сек
...
## [2026-03-17] — Сессия 19

### Проблема
Web интерфейс зависает при аварии TSA из-за конфликта доступа к SD (SPI) между Core 0 (Network Task/WebServer) и Core 1 (loop/log).

### Решение
- Любая работа с SD синхронизирована через `sdMutex` (Core 0/1).
- Убрана раздача статики через `serveStatic()` (она читает SD без мьютекса). Раздача файлов реализована вручную через `onNotFound()` под `sdMutex`.
- Исправлена ротация логов в `SDLogger`: операции `exists/remove/rename` выполняются под `sdMutex`.

### Изменённые файлы
- `SDLogger.h` — потокобезопасный доступ к SD, корректная ротация.
- `AppNetwork.cpp` — потокобезопасная раздача статических файлов с SD.
